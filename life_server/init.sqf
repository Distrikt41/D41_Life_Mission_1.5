...
D41_RentedGangBasesLiz = [];
{
	...
	if ("license_civ_base01" in _x) then {"base_marker_1" setMarkerText format[localize "STR_D41_Gang_Base", "1", (_x select 2)];};
    if ("license_civ_base02" in _x) then {"base_marker_2" setMarkerText format[localize "STR_D41_Gang_Base", "2", (_x select 2)];};
    if ("license_civ_base03" in _x) then {"base_marker_3" setMarkerText format[localize "STR_D41_Gang_Base", "3", (_x select 2)];};
    if ("license_civ_base04" in _x) then {"base_marker_4" setMarkerText format[localize "STR_D41_Gang_Base", "4", (_x select 2)];};
    if ("license_civ_base05" in _x) then {"base_marker_5" setMarkerText format[localize "STR_D41_Gang_Base", "5", (_x select 2)];};
    if ("license_civ_base06" in _x) then {"base_marker_6" setMarkerText format[localize "STR_D41_Gang_Base", "6", (_x select 2)];};
	if ("license_civ_base07" in _x) then {"base_marker_7" setMarkerText format[localize "STR_D41_Gang_Base", "7", (_x select 2)];};
}forEach _query;
...
{
	private["_dome","_rsb"];
	_dome = nearestObject [[1516.3312,14348.151,1],"Land_Offices_01_V1_F"];
...
[] spawn
{
	_PolWpnList = [["hlc_rifle_G36V",3],["RH_Hk416",3],["RH_M16A3",3],["srifle_DMR_03_F",1],["CUP_lmg_minimipara",1],["rhs_weap_M320",1]];
	{
	...
	_PolMagList = [["1Rnd_SmokeBlue_Grenade_shell", 6],["UGL_FlareWhite_F", 3],["20Rnd_762x51_Mag", 6],["30Rnd_556x45_Stanag", 30],["hlc_30rnd_556x45_SPR_G36", 16],["CUP_200Rnd_TE4_Yellow_Tracer_556x45_M249", 3],["rhs_mag_mk84", 6]];
	{
	...
	_PolItmList = [["optic_Hamr", 8],["FHQ_optic_AC12136", 8],["optic_AMS", 1],["SmokeShellBlue", 4]];
	{
...
tf_radio_channel_password = "Gdouhgd9h97dg";
...
_queryState = ["FetchState",2,true] call DB_fnc_asyncCall;
diag_log format ["::::::::: STATEQUERY:%1", _queryState]; //DEV
diag_log format ["::::::::: STATEQUERY SELECT 0:%1", _queryState select 0 select 0]; //DEV
diag_log format ["::::::::: STATEQUERY SELECT 1:%1", _queryState select 0 select 1]; //DEV
diag_log format ["::::::::: STATEQUERY SELECT 2:%1", _queryState select 0 select 2]; //DEV
D41_StateFunds = _queryState select 0 select 0;
diag_log format ["::::::::: D41_StateFunds:%1", D41_StateFunds]; //DEV
D41_StateTaxes = _queryState select 0 select 1;
diag_log format ["::::::::: D41_StateTaxes:%1", D41_StateTaxes]; //DEV
_temp = _queryState select 0 select 2;
D41_StateWages = (_temp / 100);
diag_log format ["::::::::: D41_StateWages:%1", D41_StateWages]; //DEV
publicVariable "D41_StateFunds";
publicVariable "D41_StateTaxes";
publicVariable "D41_StateWages";
