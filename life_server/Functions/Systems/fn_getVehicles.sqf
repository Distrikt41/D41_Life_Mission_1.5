...
diag_log format ["extDB2: _Gangbase: %1",_Gangbase];
...
{
	[[[],false],"life_fnc_impoundMenu",_unit,false] call life_fnc_MP;
};
...
//KARMA BASIERTES AUSPARKEN START:
	...
	if(_Gangbase != "none")then
	{
		...
		if(_Gangbase == "license_civ_base07")then			{[D41_Garage_GangBase07]		call _D41_PushItBack;};
		if(_Gangbase == "license_civ_navyseal")then			{[D41_Garage_NavySeal]			call _D41_PushItBack;};
	};
	
	
	diag_Log format ["::: %1", _KarmaCheckList];
	_KarmaVehicleList = [];
	{
		_Array = _x;
...
[[_KarmaVehicleList,false],"life_fnc_impoundMenu",_unit,false] call life_fnc_MP;