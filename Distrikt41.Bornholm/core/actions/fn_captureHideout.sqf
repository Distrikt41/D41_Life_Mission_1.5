...
if ({side _x == civilian} count playableUnits < 10) exitWith {hint localize "STR_D41_10CivHideOut";};
...
_cP = 0.01;
[[[0,1],format[localize "STR_D41_Take_Gang_hideout",name player,(group player) getVariable "gang_name" ]],"life_fnc_broadcast",true,false] call life_fnc_MP;
...
while {true} do
{
	...
	if(player distance _hideout > 15)exitWith{_hideout setVariable["inCapture",false,true];};
};
...
if(player distance _hideout > 15) exitWith {life_interrupted = false; titleText[localize "STR_GNOTF_CaptureCancel","PLAIN"]; life_action_inUse = false;_hideout setVariable["inCapture",false,true];};
...
if((player distance (getMarkerPos "Ganghideout01")) < 100)	then { "Ganghideout01" setMarkerColor "ColorBlack"; "Ganghideout01" setMarkerText format[localize "STR_D41_Hide_Bravo",(group player) getVariable "gang_name"]; hideoutOwner1 = _hideout getVariable ["gangOwner",grpNull];};
if((player distance (getMarkerPos "Ganghideout02")) < 100)	then { "Ganghideout02" setMarkerColor "ColorBlack"; "Ganghideout02" setMarkerText format[localize "STR_D41_Hide_Alpha",(group player) getVariable "gang_name"]; hideoutOwner2 = _hideout getVariable ["gangOwner",grpNull];};
if((player distance (getMarkerPos "Ganghideout03")) < 100)	then { "Ganghideout03" setMarkerColor "ColorBlack"; "Ganghideout03" setMarkerText format[localize "STR_D41_Hide_Charlie",(group player) getVariable "gang_name"]; hideoutOwner3 = _hideout getVariable ["gangOwner",grpNull];};