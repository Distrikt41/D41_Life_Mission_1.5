...
_smoke attachto [_OBJ,[0.3,-1.5,2]];
fnc_deleteObj = {
...
_fnc_gather = 
{
	...
	if(player canAdd _type) then
	{
		...
		{
			_vehicle = vehicle player;
			if(!(player canAdd _type))exitWith{_smoke spawn {sleep 1;_this call fnc_deleteObj;};};
			if(typeOf(_vehicle) != "D41_gaz66_r142_msv")exitWith{hint localize "STR_D41_Meth_KeineKueche";_smoke spawn {sleep 1;_this call fnc_deleteObj;};};
			if((speed _vehicle) > 1)exitWith{hint localize "STR_D41_Meth_AmFahren";_smoke spawn {sleep 1;_this call fnc_deleteObj;};};
			if(count(crew _vehicle) < 2)exitWith{hint localize "STR_D41_Meth_KeinTeam";_smoke spawn {sleep 1;_this call fnc_deleteObj;};};
			if(!(isEngineOn _vehicle))exitWith{hint localize "STR_D41_Meth_MotorAus";_smoke spawn {sleep 1;_this call fnc_deleteObj;};};
		...
		if(_abort)exitWith{hint localize "STR_NOTF_ActionCancel"; systemchat localize "STR_NOTF_ActionCancel";_smoke spawn {sleep 1;_this call fnc_deleteObj;};};
		if(!(player canAdd _type))exitWith{_smoke spawn {sleep 1;_this call fnc_deleteObj;};};
		if(typeOf(_vehicle) != "D41_gaz66_r142_msv")exitWith{hint localize "STR_D41_Meth_KeineKueche";_smoke spawn {sleep 1;_this call fnc_deleteObj;};};
		if((speed _vehicle) > 1)exitWith{hint localize "STR_D41_Meth_AmFahren";_smoke spawn {sleep 1;_this call fnc_deleteObj;};};
		if(count(crew _vehicle) < 2)exitWith{hint localize "STR_D41_Meth_KeinTeam";_smoke spawn {sleep 1;_this call fnc_deleteObj;};};
		...
		if(alive player)then
		{
			...
			for "_i" from 1 to _rndm do
			{
				...
				_smoke spawn {sleep 1;_this call fnc_deleteObj;};
			};
