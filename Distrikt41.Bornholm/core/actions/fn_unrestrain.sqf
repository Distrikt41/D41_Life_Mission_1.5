...
if(side player == civilian && (!("D41_Dietrich" in (magazines Player)) && !("D41_Bolzenschneider" in (magazines Player)))) exitWith {hint localize "STR_D41_UnrestrainKeinDietrich";};
D41_HC_Chance = FALSE;
if (side player == civilian) then
	{
		if ("D41_Bolzenschneider" in (magazines Player)) then 
		{
		...
		life_action_inUse = true;
			...
			if(D41_HC_Chance2 > 50) then {D41_HC_Chance = TRUE;};
			if(D41_HC_Chance) exitWith {hint localize "STR_ISTR_Lock_Failed"; life_action_inUse = false;};
...
playSound3D ["D41\D41_S\Sounds\AlifeSounds\cuff.ogg", player, false, getPosASL _unit, 2, 1, 50];
