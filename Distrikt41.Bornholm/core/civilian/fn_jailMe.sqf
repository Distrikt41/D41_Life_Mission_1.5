...
switch (true) do
{
	case (_esc) :
	{
		...
		if(playerside == civilian)then{ [[getPlayerUID player,profileName,"901"],"life_fnc_wantedAdd",false,false] call life_fnc_MP; };
	...
	case (alive player && !_esc) :
	{
		...
		[[getPlayerUID player],"life_fnc_wantedJailFine",false,false] call life_fnc_MP;
