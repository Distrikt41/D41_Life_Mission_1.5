...
if(_button isEqualTo _mapKey) then
{
	// // player tags
	// if (visibleMap) then {
		// // wenn map geschlossen werden soll, mach tags an
		// [true] call life_fnc_D41_NameTags;
	// } else {
		// // wenn map geoeffnet werden soll, mach tags aus
		// [false] call life_fnc_D41_NameTags;
	// };
...
if(_button isEqualTo 19) then
{
	...
	if(_shift && _IsWest && !isNull cursorTarget && cursorTarget isKindOf "Man" && (isPlayer cursorTarget) && (side cursorTarget in [civilian]) && alive cursorTarget && cursorTarget distance player < 3.5 && !(cursorTarget getVariable "Escorting") && !(cursorTarget getVariable "restrained") && speed cursorTarget < 1) then
	{
		D41_RestrainTarget = cursorTarget;
		[2, "Fortschritt Festnahme", {
		diag_log format ["[D41] DEBUG (keyHandler|restrain) cursorTarget: %1 -- cursorObject: %2", cursorTarget, cursorObject];
		diag_log format ["[D41] DEBUG (keyHandler|restrain) dump: geknueppelt %1 -- surrender %2 -- tazed: %3", (D41_RestrainTarget getVariable ["D41_geknueppelt", false]), (D41_RestrainTarget getVariable ["surrender", false]), (D41_RestrainTarget getVariable ["tazed", false])];
		(
			(speed player < 1) 
			&& ((D41_RestrainTarget getVariable ["D41_geknueppelt", false]) || (D41_RestrainTarget getVariable ["surrender", false]) || (D41_RestrainTarget getVariable ["tazed", false])) 
			&& !(D41_RestrainTarget getVariable ["restrained", false])
		)
		}, [], {
			// cursor target abfragen, obs immer noch der gleiche ist
			if (D41_RestrainTarget isEqualTo cursorTarget) then {
				[] call life_fnc_restrainAction; 
				[cursorTarget] joinSilent (createGroup civilian); 
				hint localize "STR_D41_arrested";
			} else {
				hint "Die Festnahme war nicht möglich. Ziel aus den Augen verloren.";
			};                
		}, {
			hint "Die Festnahme war nicht möglich.";
...
if(_button isEqualTo 24) then
{
	...
	if(_shift && !isNull cursorTarget && cursorTarget isKindOf "Man" && (isPlayer cursorTarget) && alive cursorTarget && cursorTarget distance player < 3.5 && !(cursorTarget getVariable "Escorting") && !(cursorTarget getVariable "restrained") && speed cursorTarget < 1) then
	{
		...
		[2, "Fortschritt Festnahme", {((speed player < 1) && ((cursorTarget getVariable ["D41_geknueppelt", false]) ||(cursorTarget getVariable ["surrender", false]) || (cursorTarget getVariable ["tazed", false])) && !(cursorTarget getVariable ["restrained", false]))}, [], {[] call life_fnc_restrainAction; hint localize "STR_D41_arrested";}, {hint "Die Festnahme war nicht möglich.";}] call life_fnc_D41_UIDisplayProgressBarAndExecute;
...
if(_button in actionKeys 'CommandingMenu1')then{_ButtonDisabled = true;};
if(_button in actionKeys 'CommandingMenu2')then{_ButtonDisabled = true;};
...
if(_button in actionKeys 'SelectGroupUnit4')then{_ButtonDisabled = true;};
if(_button in actionKeys 'SelectGroupUnit5')then{_ButtonDisabled = true;};
if(_button in actionKeys 'SelectGroupUnit6')then{_ButtonDisabled = true;};
if(_button in actionKeys 'SelectGroupUnit4')then{_ButtonDisabled = true;};
...
if((_button in actionKeys 'SelectGroupUnit2'))then{_ButtonDisabled = true;
	...
	{
		_veh = vehicle player;
		if((typeOf _veh) in ['D41_Offroad_URD','D41_Offroad_ADAC3','D41_Offroad_ADAC2','D41_Offroad_ADAC','D41_Offroad_Medic2','D41_Offroad_Medic','D41_S1203_Civ_02','D41_Lada_Civ_05','D41_Offroad_Pol','D41_BMW_X6M_Pol2','D41_B_Truck_01_ammo_cop','D41_Offroad_Snake_TFuel_TPS','D41_Offroad_SWAT','D41_tigr_swat','D41_Lada_Civ_05_tuned','D41_swat_hunter']) then
		{
...
if((_button in actionKeys 'SelectGroupUnit3'))then
{_ButtonDisabled = true;
...
{_ButtonDisabled = true;
	...
	if (vehicle player == player && !(player getVariable ['restrained', false]) && (animationState player) != 'Incapacitated' && !life_istazed && !(player getVariable ['surrender', false])) then
	{
		[] spawn life_fnc_d41_sitdown;
	};
}; */
if((_button in actionKeys 'SelectGroupUnit7'))then	{_ButtonDisabled = true;};
