...
{
	case west:	{
					if(_item in ["D41_medic_man_uniform","D41_TTrackerGummi","D41_AnhaengerFahrer_uniform"]) then
					{
	...
	case civilian:	{
						if(_item in ["D41_medic_man_uniform","rhs_mag_rgd5","D41_AnhaengerFahrer_uniform"]) then
						{
							[_item,false,false,false,false] call life_fnc_handleItem;
							hint localize "STR_D41_PickUP_NotAllowed";
						};
	...
	case independent:{
						if(_item in ["D41_AnhaengerFahrer_uniform"]) then
							{
								[_item,false,false,false,false] call life_fnc_handleItem;
								hint localize "STR_D41_PickUP_NotAllowed";	
						};
