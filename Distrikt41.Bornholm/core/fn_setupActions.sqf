...
switch (playerSide) do
{
	case civilian:
	{
		...
		(surfaceisWater (getPos vehicle player)) && (vehicle player isKindOf "C_Rubberboat" OR vehicle player isKindOf "C_Boat_Civil_01_F" OR vehicle player isKindOf "D41_CUP_C_Fishing_Boat_Chernarus" OR vehicle player isKindOf "D41_Trawler") && speed (vehicle player) < 2 && speed (vehicle player) > -1 && ((getPos player) distance (getMarkerPos "D41_Fischzone_01") < 500 OR (getPos player) distance (getMarkerPos "D41_Fischzone_02") < 500 OR (getPos player) distance (getMarkerPos "D41_Fischzone_03") < 500 OR (getPos player) distance (getMarkerPos "D41_Fischzone_04") < 500 OR (getPos player) distance (getMarkerPos "D41_Fischzone_05") < 500) && !life_net_dropped ']];
		life_actions = [player addAction[localize "STR_pAct_DropCrapNet",life_fnc_gatherKrabben,"",0,false,false,"",'
		(surfaceisWater (getPos vehicle player)) && (vehicle player isKindOf "D41_Trawler" OR vehicle player isKindOf "D41_CUP_C_Fishing_Boat_Chernarus") && !(vehicle player isKindOf "C_Rubberboat") && speed (vehicle player) < 15 && speed (vehicle player) > 5 && ((getPos player) distance (getMarkerPos "D41_Krabbenzone_01") < 700 OR (getPos player) distance (getMarkerPos "D41_Krabbenzone_02") < 700 OR (getPos player) distance (getMarkerPos "D41_Krabbenzone_03") < 700) && !life_net_dropped ']];
	...
	case west:
	{
		...
		//Pack Spikes
		life_actions = life_actions + [player addAction[localize "STR_ISTR_Spike_Pack",life_fnc_packupSpikes,"",0,false,false,"",
...
//Tonne einfüllen
life_actions = life_actions + [player addAction[localize "STR_D41_Meth1_einfuellen",{[true] spawn life_fnc_D41_TonneEinfuellen, D41_AntiDupe = true;},'',0,false,false,"",'
_FuelTrucks = nearestObjects[getPos player, D41_TanklasterListe,5] select 0; !isNil "_FuelTrucks" && (count(attachedObjects player) == 0) && (typeOf cursorTarget) isEqualTo "Land_MetalBarrel_F" && !(D41_AntiDupe)']];
...
//Ausweis  
life_actions = life_actions + [player addAction["Ausweis zeigen",life_fnc_Lizenzzeigen,"",1,false,true,"",'!isNull cursorTarget && cursorTarget isKindOf "Man" && player distance cursorTarget < 3.5']];
