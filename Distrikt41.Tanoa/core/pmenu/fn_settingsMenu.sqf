...
{
	if(!createDialog "SettingsMenu") exitWith {hint localize "STR_D41_Gui_Open_Error"};
};
...
private["_display","_side","_objs"];
_display = findDisplay 2900;
_side = _display displayCtrl 2971;
_objs = _display displayCtrl 2972;

if(isNil "life_sidechat" or isNil "life_revealObjects") then {
	life_sidechat = true;
...
// if(life_tagson) then {
	...
	// //_tags buttonSetAction "[false] call life_fnc_D41_NameTags; life_tagson = false; [] call life_fnc_settingsMenu;";
// } else {
	...
	// //_tags buttonSetAction "[true] call life_fnc_D41_NameTags; life_tagson = true; [] call life_fnc_settingsMenu;";
// };
