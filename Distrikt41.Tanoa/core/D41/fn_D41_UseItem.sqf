...
if(_Item == "D41_BeefJerky") then
	{
		...
		if(_sum > 100) exitWith {_sum = 100; life_hunger = _sum; player setFatigue 1; hint localize "STR_D41_ueberfressen";[0.1] spawn life_fnc_D41_TeilHeilungNahrung;;};
		life_hunger = _sum;
		[0.1] spawn life_fnc_D41_TeilHeilungNahrung;
	};
...
if(_Item == "D41_Meth") then
	{
		player removeMagazine _Item;
		life_KokaZeit = time;
		life_DrogenKonsum = life_DrogenKonsum + 1;
		titleText[localize "STR_D41_TextMeth1","PLAIN"];
		life_thirst = 200;
		life_hunger = 200;
		player setDamage 0;
		player enableFatigue false;
		waitUntil {!alive player OR ((time - life_KokaZeit) > (10 * 60))};
		titleText[localize "STR_D41_TextMeth2","PLAIN"];
		life_KokaZeit = time;
		waitUntil {!alive player OR ((time - life_KokaZeit) > (1 * 60))};
		player enableFatigue true;
		[-2.6] spawn life_fnc_D41_TeilHeilungNahrung;
	};
...
if(_Item == "D41_FalscherAusweis") then
	{
		if (life_fakeid == 1) then 
		{
			...
			hint format ["Your name is now %1.",D41_FakeName];
		};
