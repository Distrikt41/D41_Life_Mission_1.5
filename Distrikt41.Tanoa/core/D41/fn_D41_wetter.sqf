		...
		if (isDedicated) then
			{
				...
				if(D41_ChanceN >=11 && D41_ChanceN <= 35 && D41_RegenCounter == 0) then
					{
						D41_Nebel = [0,0,0]; D41_Wellen = 1; D41_Vorschau = 1; D41_Regen = 0; D41_RegenCounter = 1;
						D41_WMod = round(random 9);
				...
				if(D41_ChanceN >=11 && D41_ChanceN <= 35 && D41_RegenCounter == 1) then {D41_ChanceN = 36};
