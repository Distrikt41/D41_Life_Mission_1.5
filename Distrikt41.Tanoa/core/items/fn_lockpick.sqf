...
if(player distance _curTarget > _distance) exitWith {hint localize "STR_D41_SlenderMan"}; //Too far
...
[[0,format[localize "STR_D41_Seen_Lockpick",profileName]],"life_fnc_broadcast",true,false] call life_fnc_MP;
...
if(!_isVehicle) then {
	...
	if(_dice < 30) then {
		...
		if(playerside == civilian)then{ [[getPlayerUID player,profileName,"214"],"life_fnc_wantedAdd",false,false] call life_fnc_MP; [-50,0] call life_fnc_D41_KSys; };
		if(playerside in [west, civilian]) then 
		{
			player addMagazine "D41_Dietrich";
		};
	} else {
		if(playerside == civilian)then{ [[getPlayerUID player,profileName,"215"],"life_fnc_wantedAdd",false,false] call life_fnc_MP; [-10,0] call life_fnc_D41_KSys; };
