...
if(_mode) then {
    ...
    _lightSource2 = "#lightpoint" createVehicleLocal [0,0,0];
    ...
    switch(true) do {
        ...
        case ((typeOf _house) in ["Land_i_Shed_Ind_F"]): {_lightSource lightAttachObject [_house,[8.6,3.5,3.9]]; _lightSource2 lightAttachObject [_house,[-1.6,3.5,3.9]]; _two_lights = true;};
        default {_exit = true;};
    };
    if(_exit) exitWith {deleteVehicle _lightSource;};
    
    if(_two_lights) then {
        _lightSource2 setLightColor [250,150,50];
        _lightSource2 setLightAmbient [1,1,0.2];
        _lightSource2 setLightAttenuation [1,0,0,0];
        _lightSource2 setLightIntensity 10;
        _lightSource2 setLightUseFlare true;
        _lightSource2 setLightFlareSize 0.2;
        _lightSource2 setLightFlareMaxDistance 50;
        _house setVariable["lightSource2",_lightSource2];
    }
    else
    {
        deleteVehicle _lightSource2;
    };
    ...
    if(isNull (_house getVariable["lightSource2",ObjNull])) exitWith {};
    deleteVehicle (_house getVariable["lightSource2",ObjNull]);
};